...
class adminPostNav extends module {
	...
	public static function do_meta_box( $post_type, $type, $post ) {
		...
		if ( in_array( $post->post_status, $post_statuses ) ) {
			add_meta_box( 'adminpostnav', sprintf( __( '%s Navigation', 'wp-assistant' ), ucfirst( $post_type ) ),
				array(
					__CLASS__,
					'add_meta_box'
				), $post_type, 'side', 'core' );
...
JS;
	...
	public static function query( $type = '<', $offset = 0, $limit = 15 ) {
		...
		$sql               = $wpdb->prepare( "SELECT ID, post_title FROM $wpdb->posts WHERE post_type = %s AND post_status IN ( $post_statuses_sql ) AND $orderby $type %s ORDER BY %s %s LIMIT %d, %d;",
			$post_type, $post->$orderby, $orderby, $sort, $offset, $limit );
