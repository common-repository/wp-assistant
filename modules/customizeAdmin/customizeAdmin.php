<?php
/*
Plugin Name: 管理画面カスタマイズ
Description: 管理画面のカスタマイズをGUIで設定できるように
...
class customizeAdmin extends module {
	...
	public function __construct() {
		...
		add_action( 'admin_head', array( $this, 'change_admin_bar_logo' ), 10 );
				...
				'tabs_name' => __( 'Customize Admin', 'wp-assistant' )
		               ...
		               ->add_field(
			               array(
				               ...
				               'default' => '',
				               'section' => 'customize_admin',
				               'desc'    => __( 'Please upload the login screen background image. Will automatically be stretched. As recommended large size images.',
					               'wp-assistant' )
			               )
		               )
		               ->add_field(
			               array(
				               'id'      => 'adminbar_logo_background',
				               'title'   => __( 'アドミンバーのロゴを変更', 'wp-assistant' ),
				               'type'    => 'media',
				               'default' => '',
				               'section' => 'customize_admin',
				               'desc'    => __( '管理画面アドミンバーのロゴを変更する際にはアップロードしてください。', 'wp-assistant' )
			               )
		               )
		               ->add_field(
			               array(
	...
	public function change_admin_bar_logo() {
		$adminbar_logo = config::get_option( 'adminbar_logo_background' );
		if ( $adminbar_logo ) {
			?>
			<style>
				#wpadminbar #wp-admin-bar-wp-logo > .ab-item .ab-icon::before {

					background: url(<?php echo esc_url( $adminbar_logo ); ?>) no-repeat;
